design.mobile = {
	...
	changeview: function(control){
		var views = jQuery('#app-wrap .labView');
		var index = 0;
		var max = 0;
		views.each(function(){
			if(jQuery(this).find('img.modelImage').length == 0)
			{
				return false;
			}
		...
		jQuery('#dg-left').css('display', 'block');
...
jQuery(document).ready(function(){
	...
	if(width_site<500)
	{
		...
		jQuery('.modelImage').each(function(){
			if(jQuery(this).data('changeZoom') == undefined)
			{
				var data_mobile = jQuery(this).parents('.labView').find('.design-area').data('mobile');
				var css = {};
					css.top 	= design.convert.px(this.style.top);
					css.left 	= design.convert.px(this.style.left);
					css.width 	= design.convert.px(this.style.width);
					css.height 	= design.convert.px(this.style.height);
				jQuery(this).css({
					'top'    : (css.top * data_mobile.zoom) + 'px',
					'left'   : (css.left * data_mobile.zoom) + 'px',
					'width'  : (css.width * data_mobile.zoom) + 'px',
					'max-width'  : (css.width * data_mobile.zoom) + 'px',
					'height' : (css.height * data_mobile.zoom) + 'px',
					'max-height' : (css.height * data_mobile.zoom) + 'px'
				});
				jQuery(this).data('changeZoom', true);
			}
		});
	}
