...
class Clipart extends Controllers
{
	...
	public function index($index = 1, $cateid = 0)
	{
		...
		if (file_exists($file))
		{
			...
			if(isset($rows->arts) && try_to_count($rows->arts))
			{
	...
	* Import clipart export from store via tools
			...
			if($content !== false)
			{
				...
				if( isset($data['arts']) && $data['arts'] > 0 )
				{
					...
					if( file_exists($file) )
					{
						...
						$index 	= 0;
						if( isset($arts['arts']) && try_to_count($arts['arts']) )
						{
						...
						$arts['count'] = try_to_count($arts['arts']);
					}
					else
					{
						$arts 	= array(
	...
	public function merge($reload = 0)
	{
		...
		if(isset($data))
		{
			...
			if( try_to_count($categories) )
			{
				...
				/* add categories */
				if(try_to_count($shop_categories))
				{
		...
		if (file_exists($file))
		{
			...
			if(file_exists($file_store) && try_to_count($arts['arts']))
			{
				...
				foreach ($arts['arts'] as $key => $art)
				{
					...
					if(try_to_count($arts_ids) && in_array($id, $arts_ids)) continue;
	...
	function edit($id = 0)
	{
		...
		else
		{
			...
			if (file_exists($file))
			{
				...
				$rows 	= json_decode($str);
				if (isset($rows->arts) && try_to_count($rows->arts) > 0)
				{
	...
	public function save()
	{
		...
		if (!empty($_POST))
		{
			...
			if (isset($_POST['id']) && $_POST['id'] > 0)
			{
				$id 	= $_POST['id'];
				// update clipart
				if (isset($arts->arts) && try_to_count($arts->arts) > 0)
				{
					foreach($arts->arts as $row)
					{
						if ($row->clipart_id == $id)
	...
	public function createTree(&$list, $parent){
		...
		{
			if( isset($l->children) && try_to_count($l->children) > 0 )
			{
				$l->isFolder = 1;
			}
			$tree[] = $l;
