...
{	
			...
			{	
				foreach ( $printings as $value )
				{
					if ( $value['id'] == $id )
					{
						...
						if(array_key_exists('options_extra', $value))
						{
							$obj->options_extra	= $value['options_extra'];
						}
	...
	public function update() 
	{
		...
		if ( ! empty( $_POST ) ) 
		{
			...
			$options_extra = array();
			if ( isset( $_POST['options_extra'] ) ) $options_extra = $_POST['options_extra'];
			...
			{ # insert action
				...
				$obj->options_extra	= $options_extra;
				$obj->values		= $values;
				$printings[] 		= $obj;
			...
			else # update action
			{
				...
				foreach( $printings as &$value )
				{
					if( $value['id'] == $id )
					{
						...
						$value['options_extra']	= $options_extra;
						$value['values'] 		= $values;
						break;
					}
				}
	...
	{
		$out  = array();
		$rows = try_to_count( $arr );
		if ( $rows > 0 )
		{
			$cols = try_to_count( $arr[0] );
			for( $i = 0; $i < $rows; $i++ )
			{
				for( $j = 0; $j < $cols; $j++ )
				{
		...
		if ( is_dir( $folder ) )
		{
			...
			if ( $handle = opendir( $folder ) )
			{
				while ( false !== ( $entry = readdir( $handle ) ) )
				{
					if( ! in_array( $entry, array(".","..") ) && file_exists( $folder . DS . $entry ) )
					{
						if(try_to_count(explode('.json', $entry)) == 1) continue;
