...
{	
	...
	*/
	...
	{
		if(try_to_count($ids) ==0) return false;
		...
		if(file_exists($file))
		{
			...
			if ($content !== false)
			{
				...
				for($i=0; $i<try_to_count($categories); $i++)
				{
					...
					if (isset($category['children']) && try_to_count($category['children']))
					{
						for($j=0; $j<try_to_count($category['children']); $j++)
						{
	...
	*/
	protected function treeCategories($categories)
	{
		...
		if(try_to_count($categories))
		{
			...
			// find children
			if(try_to_count($data) && try_to_count($temp))
			{				
				for($i=0; $i<try_to_count($data); $i++)
				{
					$data[$i]['children'] = array();
					if(try_to_count($temp))
					{
	...
	*/
	public function sortCategories($type = 'art')
	{
		if (isset($_POST['categories']))
		{
			...
			$data 		= array();
			for($i=0; $i<try_to_count($temp); $i++)
			{
				$parent = explode(':', $temp[$i]);
				if(try_to_count($parent) > 1)
				{
					$data[$parent[0]]	= array();
					$params 			= explode(',', $parent[1]);
					for($j=0; $j<try_to_count($params); $j++)
					{
						if ($params[$j] != '')
			...
			if ($content != false)
			{
				...
				{					
					...
					{						
						...
						if(isset($categories[$i]['children']) && try_to_count($categories[$i]['children']) > 0)
						{
							for($j=0; $j<try_to_count($categories[$i]['children']); $j++)
							{
	...
	*/
	public function arts($page = 1)
	{
		...
		if (!isset($search['subcate'])) {
			$search['subcate'] = $search['cate'];
		}
		$data['search']	= $search;
		$data['info']	= array();
		...
		$number = (int) (try_to_count($arts)/$data['limit']);
		if(try_to_count($arts)%$data['limit'])
		{
			$number	= $number + 1;
		}
	...
	*/
	public function edit($type = 'arts', $id = 0)
	{
		...
		{
			$post = $_POST['data'];
			if (!isset($post['cate_id']) || $post['cate_id'] == null || empty($post['cate_id'])) {
				$post['cate_id'] = $post['parent_cate_id'];
			}
	...
	public function ideas($page = 1)
	{
		...
		if(isset($_POST['search']))
		{
			...
			if($search['cate_id'] > 0)
			{
				...
				for($i=0; $i<try_to_count($categories); $i++)
				{
					if( $categories[$i]['id'] == $search['cate_id'] && isset($categories[$i]['children']) && try_to_count($categories[$i]['children']) > 0 )
					{
		...
		$number = (int) (try_to_count($ideas)/$data['limit']);
		if(try_to_count($ideas)%$data['limit'])
		{
			$number	= $number + 1;
		}
		$data['page_number']	= $number;
			...
			if(try_to_count($types))
			{
				...
				foreach($ideas_types as $type_id => $idea_ids)
				{
					...
					else	// remove
					{
						...
						{
							$ids 	= array();
							for($i=0; $i<try_to_count($idea_ids); $i++)
							{
								if($id != $idea_ids[$i])
								{
									$ids[]	= $idea_ids[$i];
