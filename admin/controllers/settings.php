...
class Settings extends Controllers
{
	public function index()
	{
		...
		if(try_to_count($data['languages']))
		{
	...
	public function editLang($admin = 'admin')
	{
		...
		if(!empty($_POST['language']))
		{
			...
			$strs_lang = base64_decode($_POST['language']);
			$strs_lang = mb_convert_encoding($strs_lang, 'UTF-8');
			$data = json_decode($strs_lang, true);
	...
	public function pageColor($segment = 0)
	{
		...
		if(isset($colors['colors']))
		{
			...
			if($perpage < try_to_count($sort))
				$data['page'] = $perpage;
			else
				$data['page'] = 0;
			$data['colors'] = $pagecolor;
			$data['total'] = try_to_count($sort);
	...
	public function editFont($id = '')
	{
		...
		$up = new upload();
		$data['file_size_max'] = $up->file_size;
		...
		if(isset($_POST['title']) && $_POST['title'] != '')
		{
			...
			if($id != '')
			{
				if(try_to_count($font) && $data['error'] == '')
				{
					$font_out = array();
					foreach($font as $key=>$val)
					{
						if($val['id'] == $id && $val['type'] == '')
						{
							...
							if($data['error'] == '' && try_to_count($result) == 3)
							{
								$filename = array(
	...
	public function removeFont()
	{
		...
		foreach($_POST['checkb'] as $id)
		{
			if(isset($fonts['fonts']['fonts'][$id]))
			{
				// unset catefonts.
				$cate_id = $fonts['fonts']['fonts'][$id]['cate_id'];
				if(isset($fonts['fonts']['cateFonts'][$cate_id]['fonts']) && try_to_count($fonts['fonts']['cateFonts'][$cate_id]['fonts']))
				{
