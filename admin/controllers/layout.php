...
class Layout extends Controllers
{
	...
	*/
	public function setDefault($id = '')
	{
		...
		if($layouts != false && try_to_count($layouts))
		{
	...
	*/
	public function remove($id)
	{
		$this->dg 	= new dg();
		$layouts = $this->getLayouts();
		if( $layouts != false && try_to_count($layouts) && isset($layouts[$id]) )
		{
	...
	protected function updateSetting($data)
	{
		...
		if( isset($data['options']) && try_to_count($data['options']) )
		{
	...
	*/
	public function edit($id = '')
	{
		...
		else
		{
			...
			{
				$data['setting']	= $file;
				if( isset($data['layout']['options']) && try_to_count($data['layout']['options']) )
				{
					$data['settings'] = array();
	...
	*/
	...
	protected function getThemes()
	{
		...
		if (file_exists($path))
		{
			...
			if (try_to_count($folders) == 0)
				return false;
						
			$list = array();
			for($i=0; $i<try_to_count($folders); $i++)
			{
		...
		if (isset($list) && try_to_count($list) > 0)
		{
			$themes = array();
