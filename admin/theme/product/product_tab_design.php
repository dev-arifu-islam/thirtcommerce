...
if (isset($product->design))
{
	...
	if( isset($design->color_hex) && try_to_count($design->color_hex)) 
	{
		for( $i=0; $i<try_to_count($design->color_hex); $i++)
		{
...
<script type="text/html" id="html_color">
	<div class="color-group">
		...
		<div class="color-group-right">
			<input type="text" name="product[design][color_title][]" class="form-control" placeholder="Color title">
	...
	<div class="product-row">
		...
		<div class="product-col-right">
			<div class="product-col-content">
				...
				<?php for($ii=0; $ii< try_to_count($product_colors); $ii++ ) { ?>
					...
					<div class="color-group">
						<div class="color-group-left">
							<div class="product-color">
								...
								for($jc=0; $jc < try_to_count($colors_hex); $jc++)
								{
								?>
...
if(try_to_count($design_front)) {
	for($ii=0; $ii<try_to_count($design_front); $ii++)
	{
		echo '<input type="hidden" class="input-old-data" data-view="front" value="'.$design_front[$ii].'">';
	}
}
if(try_to_count($design_back)) {
	for($ii=0; $ii<try_to_count($design_back); $ii++)
	{
		echo '<input type="hidden" class="input-old-data" data-view="back" value="'.$design_back[$ii].'">';
	}
}
if(try_to_count($design_left)) {
	for($ii=0; $ii<try_to_count($design_left); $ii++)
	{
		echo '<input type="hidden" class="input-old-data" data-view="left" value="'.$design_left[$ii].'">';
	}
}
if(try_to_count($design_right)) {
	for($ii=0; $ii<try_to_count($design_right); $ii++)
	{
