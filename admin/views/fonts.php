			...
			<div id="refresh">
								...
								<tr>
									...
									<td class="center">
										<?php
											...
											{
												$filename = json_decode($val['filename'], true);
												if(try_to_count($filename))
												{
													$file_name = '';
													$i = 0;
				...
				<div class="row">
					<div class="dataTables_paginate paging_bootstrap" style="float: right;">
						<div class="col-md-12">
							<?php
								if(try_to_count($data['fonts']) > 10)
								{
									$count = try_to_count($data['fonts'])/10;
									if($count > (int)$count)
										$count = (int)$count + 1;
									if($count > 5)
									{
									...
									if($pageall)
										echo '<li><a href="javascript:void(0);" onclick="pagination('.(try_to_count($data['fonts'])-10).')"><span aria-hidden="true">&raquo;</span></a></li>';
									echo '</ul>';
								}
							?>
						</div>
