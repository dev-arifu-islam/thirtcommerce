			...
			if (try_to_count($design))
			{
				...
				if (isset($area->$position) && $area->$position != '')
				{
					...
					foreach($items as $item)
					{
						...
						if (isset($item->clipar_type) && isset($item->price) && empty($item->clipar_paid) && $item->price > 0)
						{
						...
						$item->svg 	= str_replace('affTextFilteritem', 'affTextFilteritem-'.$file, $item->svg);
						$item->svg 	= str_replace('myClipPathitem', 'myClipPathitem-'.$file, $item->svg);
						// remove transform
						if($file != 'svg')
						{
							$str_temp = explode('transform="', $item->svg);
							if(try_to_count($str_temp) > 1)
							{
						...
						if (try_to_count($count) > 2)
						{
							$item->svg = $count[0]. ' xmlns:xlink="http://www.w3.org/1999/xlink"';
							for($iz=1; $iz < try_to_count($count); $iz ++)
							{
								$item->svg .= $count[$iz];
							}
						}					
						...
						if ( isset($item->file) && isset($item->file->type) && $item->file->type == 'image')
						{
							...
							if (isset($links[1][0]))
							{
								...
								elseif (isset($item->file_name) && isset($item->url))
								{
					...
					{						
						...
						if (isset($elms) && try_to_count($elms) > 0)
						{
							function cmp1($a, $b)
							{
								return $a['zIndex'] - $b['zIndex'];
					...
					if(is_dir($path))
					{
						...
						$check = false;
						if(is_array($svgFiles) && try_to_count($svgFiles))
						{
					...
					else
					{
						...
						foreach ($books as $book)
						{
							...
							$x 	= (float) $book->getAttribute('x') * $zoom;
							$book->setAttribute('x', $x);
							
							$y 	= (float) $book->getAttribute('y') * $zoom;
						...
						{
							$style_img = $img->getAttribute('style');
							$clip_path = $img->getAttribute('clip-path');
							if(strpos($style_img, 'clip-path') === false && $clip_path == '')
							{
