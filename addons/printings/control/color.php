...
function getPricePrinting($printing, $data, $quantity)
{
	...
	if ( isset($printing->values) && is_object($printing->values) && isset($data['colors']) )
	{
		...
		if (is_object($colors))
		{
			...
			{
				if ( is_array($color) && try_to_count($color) == 0) continue;
				...
				if ( isset($values[$view]) && isset($values[$view]['prices']) && isset($values[$view]['quatity']) )
				{
					...
					//get color
					$count_color = try_to_count($color);
					if ( isset($prices[$count_color-1]) )
					{
						$price = $price + $count_color * str_replace(',','.',$prices[$count_color-1]);
					}
					else if ($count_color > try_to_count($prices) )
					{
						$count_price = try_to_count($prices);
						$price = $price + $count_color * str_replace(',','.',$prices[$count_price - 1]);
					}					
				}
			}
		}
