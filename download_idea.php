...
if ( isset($_GET['key']) )
{
	...
	elseif ($is_admin == 0)
	{
		...
		if ($user_id == false or try_to_count($user_id) == 0) return false;
				...
				if (isset($area->$position) && $area->$position != '')
				{
					...
					foreach($items as $item)
					{
						...
						if ( isset($item->file) && isset($item->file->type) && $item->file->type == 'image' && empty($item->clipar_paid) )
						{
							...
							if (isset($links[1][0]))
							{
								...
								if (strpos($link, strtolower('image/PNG;base64')) === false)
								{
					...
					else
					{
						...
						foreach ($books as $book)
						{
							...
							$book_x = $book->getAttribute('x');
							if (empty($book_x) || $book_x == null) {
								$book_x = 0;
							}
							$x 	= (float)$book_x * (float)$zoom;
							$book->setAttribute('x', $x);

							$book_y = $book->getAttribute('y');
							if (empty($book_y) || $book_y == null) {
								$book_y = 0;
							}
							$y 	= (float)$book_y * (float)$zoom;
